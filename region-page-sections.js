...
function makeCarousel(id, els) {
  /* Returns { html, id, els } — els are injected after innerHTML assignment */
  return {
    html: `
...
function buildResultsSection(live, upcoming, recent, allRows) {
  const regionSlug = REGION.replace(/\s/g, "-");
  const liveC     = makeCarousel(`bsf-live-${regionSlug}`,     live.map(m => makeCard(m, "live", null)));
  const upcomingC = makeCarousel(`bsf-upcoming-${regionSlug}`, upcoming.map(m => makeCard(m, "upcoming", allRows)));
  const recentC   = makeCarousel(`bsf-recent-${regionSlug}`,   recent.map(m => makeCard(m, "recent", null)));

  /* Determine which tabs to show — always show Recent, conditionally show Live/Upcoming */
  const tabs = [];
  if (live.length)     tabs.push({ key: "live",     label: live.length + " LIVE",   id: liveC.id });
  if (upcoming.length) tabs.push({ key: "upcoming", label: "UPCOMING",              id: upcomingC.id });
  tabs.push(                     { key: "recent",   label: "RESULTS",               id: recentC.id });

  const defaultTab = tabs[0].key;

  const tabHTML = tabs.map(t => `
    <button class="bsf-results-tab${t.key === defaultTab ? " active" : ""}"
      data-tab="${t.key}"
      onclick="(function(btn){
        document.querySelectorAll('.bsf-results-tab').forEach(b=>b.classList.remove('active'));
        btn.classList.add('active');
        document.querySelectorAll('.bsf-results-panel').forEach(p=>{
  ...
  const panelHTML = [
    { key: "live",     c: liveC,     show: live.length > 0 },
    { key: "upcoming", c: upcomingC, show: upcoming.length > 0 },
    { key: "recent",   c: recentC,   show: true }
  ].map(({ key, c, show }) => show ? `
    <div class="bsf-results-panel" data-panel="${key}"
      style="display:${key === defaultTab ? "" : "none"};">
      ${c.html}
    </div>` : "").join("");

  const carousels = [liveC, upcomingC, recentC].filter((_, i) =>
    [live.length > 0, upcoming.length > 0, true][i]
  );

  return {
    html: `
      <div class="bsf-region__section">
        <div class="bsf-region__section-label">Results</div>
      </div>
      <div class="bsf-results-tabs">${tabHTML}</div>
      ${panelHTML}`,
    carousels
  };
}
...
/* ── ASSEMBLE PAGE ──────────────────────────────────────── */
const resultsSection = buildResultsSection(live, upcoming, recent, allRows);

root.innerHTML = [
  BIO ? `<div class="bsf-region__bio">${BIO}</div>` : "",
  (live.length || upcoming.length || recent.length) ? resultsSection.html : "",
  champHTML,
  newsHTML,
].filter(Boolean).join("\n");

/* Insert card DOM elements into carousels */
resultsSection.carousels.forEach(c => {
  if (!c.id || !c.els.length) return;
...
if (live.length) {
  setInterval(async () => {
    try {
      ...
      const liveCarousel = document.getElementById(`bsf-live-${REGION.replace(/\s/g,"-")}`);
      if (liveCarousel && freshLive.length) {
        liveCarousel.innerHTML = "";
        freshLive.forEach(m => liveCarousel.appendChild(makeCard(m, "live", null)));
      }
    } catch(e) {}
  }, 60000);
}
  ...
  window.__basofuSectionsRunning = false; // allow retry
